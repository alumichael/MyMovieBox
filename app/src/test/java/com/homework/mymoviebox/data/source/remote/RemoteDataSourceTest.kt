...
class RemoteDataSourceTest {
    ...
    private val remoteDataSource: RemoteDataSource = RemoteDataSource(services)
    ...
    fun getMovies() = runBlocking {
        // Mocked response
        val pageNo = 1
        val expectedResponse = listOf(MovieResponse(
            id = 1,
            backdrop = "path",
            title = "Movie Title",
            poster = "path",
        ...
        val result = remoteDataSource.getMovies(pageNo)
    ...
    fun searchMovie() = runBlocking {
        ...
        // Invoke the method under test
        val result = remoteDataSource.searchMovie(query,pageNo)
    ...
    fun getMovie() = runBlocking {
        ...
        // Invoke the method under test
        remoteDataSource.getMovie(movieId).collectLatest {
                value ->
            assertEquals(Response.Success::class.java, value::class.java)
        }
